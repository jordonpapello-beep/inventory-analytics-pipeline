...
> NOTE: **All datasets in this repository are simliar to but NOT actual company data.** To preserve the protection of the company's data, all original inventory data seen in this project have been altered in a way that preserves trends, but hides the proprietary data. This was accomplished by scaling the data linearly and then adding a little random noise to each order and on hand value (thru is then calculated based on those two new values). Therefore all of the trends represented in plots will maintain the same shape just at a different scale.
